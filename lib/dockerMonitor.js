...
function processStats (chunk, enc, cb) {
  ...
  metrics.forEach(function (key) {
    agg.update(stats['read'], key, stats[key], useDivForAggregation[key]||false)
  })
  // setTimeout (cb, 200)
  cb()
}
...
function getDockerStats (dockerStatsCallback) {
  stats(opts).pipe(through.obj(processStats))
}
...
ioMetrics.forEach (function (key) {
  var item = {}
  item[key] = true
  useDivForAggregation.push (item)
})
