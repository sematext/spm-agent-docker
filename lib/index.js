...
if (process.env.LOGSENE_TOKEN) {
  ...
  if (process.env.JOURNALD_RECEIVER_PORT) {
     var journaldLogger = require('./tcpJsonToLogsene')
     console.log('Listening for "journalctl -o json | nc localhost PORT" on exposed port ' + process.env.JOURNALD_RECEIVER_PORT)
