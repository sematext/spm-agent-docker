...
DockerLogsene.prototype.logLine = function (logline, data, next) {
  ...
  var lineParser = {
    chunk: data,
    line: process.env.REMOVE_ANSI ? logline.replace(
    ...
    parse: function () {
      ...
      function (err, logObject) {
        ...
        if (logObject) {
          ...
          self.logger.log(String((logObject.level || logObject.lvl || logObject.severity || level)), logObject.message, logObject)
          self.debugLog('Stored Log: ' + JSON.stringify(logObject))
        }
      })
    }
  }
