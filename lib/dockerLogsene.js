...
function DockerLogsene () {
  ...
  logger.on('error', function (err) {
    logger.log('error', 'Error in logsene-js: ' + err, err)
  })
  var opts = {
    json: false,
    docker: null,
  ...
  var logStream = through.obj(function (chunk, enc, cb) {
  ...
  logStream.on('error', function (err) {
    logger.log(err, 'Error in log stream: ' + err)
  })
  loghose(opts).pipe(logStream)
}
