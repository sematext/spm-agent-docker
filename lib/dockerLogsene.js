...
DockerLogsene.prototype.getLogObject = function (line, container, cbf) {
  ...
  this.debugLog('Logs from Docker API (' + lines.length + ' lines)')
  this.debugLog('Logs from Docker API (content): ' + lines)
  var self = this
    ...
    parse: function () {
      ...
      function (err, logObject) {
        ...
        if (logObject) {
          ...
          self.logger.log(new String( (logObject.level||logObject.lvl||logObject.severity||level)), logObject.message, logObject)
        }
      })
    }
  }
