...
Aggregator.prototype.update = function (timestamp, name, value, calcDiff) {
  // console.log (timestamp, name, value)
  if (this.metrics[name] === undefined) {
    this.metrics[name] = new Measured.Histogram()
    this.metrics[name].update(0, timestamp)
    this.metrics[name].lastStatsValue = value
  }
  var lastValue = value
  if (calcDiff)
  {
    var lastValue = value-this.metrics[name].lastStatsValue
    this.metrics[name].update(lastValue, timestamp)
  } 
  this.metrics[name].lastStatsValue = lastValue
}
